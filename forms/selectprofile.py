...
class FailedToLoadProfile(Exception):
    pass
...
class SelectProfile(npyscreen.ActionForm):
    ...
    def load_profile(profile_name, password=None):
        ...
        try:
            ...
            # raise FailedToLoadProfile

    def on_ok(self):
        if self.group_selected.value:
            idx = self.group_selected.value[0]
            profile = list_of_profile_names[idx]
            if profiles_properties[profile].get('encrypted'):
                if self.profile_password.value:
                    try:
                        self.load_profile(profile, self.profile_password.value)
                    except FailedToLoadProfile:
                        npyscreen.notify_confirm("Something went wrong, we failed to load profile. Make sure you entered correct password", wide=True, editw=1)
                        return
                else:
